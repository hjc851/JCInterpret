...
fun main(args: Array<String>) {
    val title = args[0]
    val traceRoot = Paths.get(args[1]).resolve(title)
    val graphRoot = Paths.get(args[2]).resolve(title)
    val out = Paths.get(args[3]).resolve(title)
    ...
    val conditionalOut = out.resolve("conditional")
    val traceOut = out.resolve("tracefeature")
    val graphOut = out.resolve("graphfeature")
    Files.createDirectories(conditionalOut)
    Files.createDirectories(traceOut)
    Files.createDirectories(graphOut)

    println("Extracting conditional features ...")
    ConditionalFeatureExtractor.extract(traceRoot, conditionalOut)

    println("Extracting trace features ...")
    TraceFeatureExtractor.extract(traceRoot, traceOut)

    println("Extracting graph features ...")
    GraphFeatureExtractor.extract(graphRoot, graphOut)
